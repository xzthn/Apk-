...
public class GlobalGatt {
    ...
    public boolean isConnected(final String address) {
        if (D) {
            Log.d(TAG, "isConnected, addr: " + address + ", mConnectionState.get(address): " + mConnectionState.get(address));
        }
    ...
    public boolean connect(final String address, final BluetoothGattCallback callback) {
        ...
        if (mBluetoothDeviceAddresss.contains(address) && isConnected(address) && (!(mCallbacks.get(address).contains(callback)))) {
            if (D) {
                Log.d(TAG, "if connect, an other want connect. addr: " + address);
            }
            // register a callback
    ...
    public void closeBluetoothGatt(final String addr) {
        if (D) {
            Log.d(TAG, "closeBluetoothGatt, addr: " + addr + ", mBluetoothGatts.get(addr): " + mBluetoothGatts.get(addr));
        }
    ...
    public void closeAll() {
        if (D) {
            Log.d(TAG, "closeAll, mBluetoothDeviceAddresss.size(): " + mBluetoothDeviceAddresss.size());
        }
        for (String addr : mBluetoothDeviceAddresss) {
            if (D) {
                Log.w(TAG, "close all of addr: " + addr);
            }
        }
        // disconnect and close the gatt
        for (String addr : mBluetoothDeviceAddresss) {
            if (D) {
                Log.d(TAG, "close all of addr: " + addr);
            }
            close(addr);
        }
    }
    ...
    public boolean readCharacteristic(final String addr, BluetoothGattCharacteristic characteristic) {
        ...
        if (D) {
            Log.d(TAG, "readCharacteristic, addr: " + addr);
        }
    ...
    public boolean writeCharacteristic(final String addr, BluetoothGattCharacteristic characteristic) {
        ...
        if (D) {
            Log.d(TAG, "writeCharacteristic, addr: " + addr);
        }
    ...
    public boolean setCharacteristicNotification(final String addr, BluetoothGattCharacteristic characteristic,
        ...
        if (D) {
            Log.d(TAG, "setCharacteristicNotification, addr: " + addr);
        }
    ...
    public boolean readCharacteristicSync(final String addr, BluetoothGattCharacteristic characteristic) {
        if (D) {
            Log.d(TAG, "readCharacteristicSync");
        }
        ...
        synchronized (mGattCallbackLock) {
            try {
                ...
                if (D) {
                    Log.e(TAG, "readCharacteristicSync Sleeping interrupted, e:" + e);
                }
            }
        }
        return true;
    ...
    public String getDeviceName(final String addr) {
        if (mBluetoothGatts.get(addr) == null) {
            if (D) {
                Log.e(TAG, "bluetooth gatt is null, addr: " + addr);
            }
            return null;
        }
    ...
    public void registerCallback(final String addr, BluetoothGattCallback callback) {
        if (D) {
            Log.d(TAG, "registerCallback, addr: " + addr);
        }
        // register a callback
        if (mCallbacks.get(addr) == null) {
            if (D) {
                Log.i(TAG, "line: 537 mCallbacks.get(addr) == null, addr: " + addr);
            }
            ArrayList<BluetoothGattCallback> c = new ArrayList<>();
            c.add(callback);
            mCallbacks.put(addr, c);
            if (D) {
                Log.i(TAG, "line:543 mCallbacks.get(addr) = " + mCallbacks.get(addr) + ". mCallbacks.size(): " + mCallbacks.size() + ", addr: " + addr);
            }
            Iterator key = mCallbacks.keySet().iterator();
            while (key.hasNext()) {
                if (D) {
                    Log.e(TAG, "mCallbacks list: " + key.next());
                }
            }
            return;
        }
        ...
        if (!(mCallbacks.get(addr).contains(callback))) {
            ...
            mCallbacks.put(addr, c);
            if (D) {
                Log.d(TAG, "mCallbacks.get(addr).contains(callback). mCallbacks.size(): " + mCallbacks.size() + ", addr: " + addr);
            }
            Iterator key = mCallbacks.keySet().iterator();
            while (key.hasNext()) {
                if (D) {
    ...
    public void unRegisterCallback(final String addr, BluetoothGattCallback callback) {
        if (D) {
            Log.d(TAG, "unRegisterCallback, addr: " + addr);
        }
        if (mCallbacks.get(addr) == null) {
            if (D) {
                Log.d(TAG, "unRegisterCallback, mCallbacks.get(addr) == null");
            }
            return;
        }
        ...
        if (mCallbacks.get(addr).contains(callback)) {
            if (D) {
                Log.d(TAG, "unRegisterCallback, unregister a callback");
            }
            // unregister a callback
            ArrayList<BluetoothGattCallback> c = mCallbacks.get(addr);
            c.remove(callback);
    ...
    class GattCallback extends BluetoothGattCallback {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    ...
                    if (D) {
                        Log.d(TAG, "mBluetoothGatts.get(addr) = " + mBluetoothGatts.get(addr) + ". mBluetoothGatts.size(): " + mBluetoothGatts.size() + ", addr: " + addr);
                    }
                    Iterator key = mBluetoothGatts.keySet().iterator();
                    while (key.hasNext()) {
                        if (D) {
                            Log.e(TAG, "mBluetoothGatts list: " + key.next());
                        }
                    }
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            String addr = gatt.getDevice().getAddress();
            if (D) {
                Log.d(TAG, "onCharacteristicChanged, addr: " + addr);
            }
            if (D) {
                Log.d(TAG, "onCharacteristicChanged, mCallbacks size: " + mCallbacks.size() + ", mCallbacks.get: " + mCallbacks.get(addr).size());
            }
            // tell all the callback
            for (BluetoothGattCallback callback : mCallbacks.get(addr)) {
        ...
        public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
            if (D) {
                Log.e(TAG, "onMtuChanged new mtu is " + mtu);
            }
            if (D) {
                Log.e(TAG, "onMtuChanged new status is " + String.valueOf(status));
            }
            String addr = gatt.getDevice().getAddress();
            // tell all the callback
            for (BluetoothGattCallback callback : mCallbacks.get(addr)) {
